...
2. What is the primary difference between IaC and CaC?
   - **A.** IaC configures OS settings; CaC builds networks
   - **B.** IaC provisions resources; CaC configures them
   - **C.** CaC is only for AWS
   - **D.** IaC cannot be versioned
...
9. YAML is preferred over JSON for authoring IaC mainly because it:
   - **A.** Is faster at runtime
   - **B.** Supports comments and is more readable
   - **C.** Requires no indentation
   - **D.** Cannot contain lists
...
16. In JSON, which statement is true?
   - **A.** Comments are allowed with `#`
   - **B.** Keys need no quotes
   - **C.** No comments; braces delimit
   - **D.** Indentation defines structure
