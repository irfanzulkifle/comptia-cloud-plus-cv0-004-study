...
C. Tracing
D. Alerting
**Answer: C** — distributed tracing follows a request across services to find the bottleneck.
...
**Answer: B** — immutable/archive retention satisfies long-term, tamper-proof compliance.
...
**Answer: B** — aggregation brings scattered logs into one queryable store.
...
C. Tracing
D. Logging
**Answer: B** — saturation (how "full" a resource is) completes the four.
...
D. Tracing
**Answer: C** — triage should collapse related alerts into one incident.
...
C. Counter
D. Summary
**Answer: C** — counters monotonically increase (e.g., requests served).
...
C. X-Ray
D. SNS
**Answer: C** — X-Ray maps request flows and latency per segment.
...
C. X-Ray
D. Lambda
**Answer: B** — CloudTrail logs who-did-what-when for governance.
...
**Answer: B** — tuning thresholds and suppression reduces low-value pages.
...
D. Always encrypted
**Answer: B** — consistent fields enable fast querying at scale.
...
**Answer: B** — gauges go up or down and show current state.
...
C. Triage
D. Monitoring
**Answer: B** — retention tiers balance cost vs. access speed vs. compliance.
...
D. Retention
**Answer: C** — response acts on the triaged, routed alert.
...
**Answer: B** — propagating trace IDs links spans across boundaries.
...
C. Retention
D. Saturation
**Answer: D** — saturation is a monitoring/golden-signal concept.
...
C. Collection
D. Tracing
**Answer: B** — alarms trigger notifications/actions = alerting.
...
C. Total uptime
D. Error counts
**Answer: B** — histograms capture distributions enabling percentiles.
...
**Answer: B** — write-once storage supports forensic integrity.
...
**Answer: B** — severity routing prevents unnecessary off-hours pages.
...
C. HOW much it costs
D. WHICH region
**Answer: B** — alerts tell you when human action is needed.
