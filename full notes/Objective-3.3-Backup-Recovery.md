...
C. Full
D. Synthetic
**Answer: C** — a full backup alone contains everything; no chain needed.
...
D. Seven days ago
**Answer: B** — incremental = since the most recent backup (full or incremental).
...
**Answer: B** — differential resets only with a new full; it re-copies since that full.
...
D. Unencrypted
**Answer: B** — off-site location protects against site/region loss.
...
C. In-place only
D. Full rebuild
**Answer: B** — granular restores one item without restoring everything.
...
C. Bulk recovery
D. Differential
**Answer: B** — parallel recovery validates in an alternate environment first.
...
C. Encryption
D. Recovery type
**Answer: B** — more frequent backups (schedule) reduce max data loss (RPO).
...
C. Test it
D. Retain it longer
**Answer: B** — without the key, encrypted backup data cannot be decrypted.
...
C. Replication
D. Retention
**Answer: B** — only test restores confirm recoverability.
...
C. Load balancers
D. Schedules
**Answer: B** — checksums/hashes prove the backup is complete and unaltered.
...
D. Always off-site
**Answer: B** — sync replication has ~zero loss but is limited by latency.
...
C. Slow restores
D. Encryption cost
**Answer: B** — one off-site copy survives localized catastrophe.
...
C. Replication
D. Granular recovery
**Answer: B** — retention defines how long copies are kept (compliance).
...
C. RDS replica
D. KMS key
**Answer: B** — AWS Backup plans define schedule, retention, replication.
...
C. Region level
D. User level
**Answer: B** — only changed blocks are stored (block-level incremental).
...
**Answer: B** — Multi-AZ sync replication enables automatic failover.
...
C. Encryption
D. Schedule drift
**Answer: B** — WORM/immutable backups can't be altered by attackers.
...
C. A single row
D. A config typo
**Answer: B** — bulk restores whole systems for major recovery.
...
**Answer: B** — chain dependency means any broken link loses subsequent data.
...
D. Skip validation
**Answer: B** — in-place overlays live data with no isolation/validation.
