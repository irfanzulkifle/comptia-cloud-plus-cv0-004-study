...
1. New instances boot with 169.254.x.x addresses and cannot reach the network, though existing instances work. What is the most likely cause?
   ...
   D. Network overlap
   Answer: B — Self-assigned APIPA addresses indicate the instance never received a DHCP lease.
...
2. `ping 8.8.8.8` succeeds but `ping google.com` fails. What is the cause?
   ...
   Answer: C — IP connectivity works, so the failure is at name resolution (DNS), not the path.
...
3. An app server cannot connect to an RDS instance in the same VPC; both are running and connections time out. Most likely cause?
   ...
   D. Latency
   Answer: B — A timed-out connection to a running resource is almost always a dropped firewall/SG/NACL rule.
...
4. A web page loads in 3 seconds because the database is in another region, but large file uploads are fast. Cause?
   ...
   Answer: C — Slow per-request responses with fine bulk transfer point to cross-region round-trip latency.
...
5. A 500 GB nightly sync across a VPN takes 9 hours and stalls. Most likely cause?
   ...
   C. DNS failure
   D. DHCP failure
   Answer: B — A large transfer that crawls/stays slow is a throughput/bandwidth bottleneck, not delay.
...
6. After a security policy enforces TLS 1.2 minimum, a legacy agent logging "handshake failure" stops connecting. Cause?
   ...
   D. Routing issue
   Answer: B — The client and server share no common TLS version; align the protocol on both ends.
...
7. On a provider's announced cutoff date, all TLS 1.0 clients fail while TLS 1.2 clients work. Cause?
   ...
   D. Latency
   Answer: B — A scheduled provider-wide removal of the old protocol affects everyone still using it.
...
8. A subnet can no longer launch instances, returning "insufficient free addresses." Cause?
   ...
   Answer: C — The subnet's IP pool is full; add a CIDR or larger subnet.
...
9. A VPC peering fails because both VPCs use 10.0.0.0/16. Cause?
   ...
   C. Bandwidth issue
   D. Deprecation
   Answer: B — Colliding CIDRs make routing ambiguous; re-address one side non-overlapping.
...
10. Private-subnet instances cannot reach the internet, but public-subnet instances can, with identical security groups. Cause?
    ...
    C. DNS failure
    D. DHCP failure
    Answer: B — With identical SGs, the difference is the route table: the private subnet lacks a route to the NAT gateway.
...
11. Traffic destined for an on-prem network is being sent to the internet instead. Cause?
    ...
    D. Scope exhaustion
    Answer: B — A route exists but points to the wrong target (igw instead of VPN/transit gateway).
...
12. A device is on the wrong network segment and can't see its VLAN peers though IP config is correct. Cause?
    ...
    C. Bandwidth issue
    D. DNS failure
    Answer: B — Correct L3 config but isolated/wrong segment indicates a Layer-2 VLAN tag or access/trunk mistake.
...
13. Authentication to a service fails intermittently with clock-skew errors, though connectivity is fine. Cause?
    ...
    C. Routing issue
    D. Bandwidth issue
    Answer: A — Clock skew breaks time-based auth; sync NTP across hosts.
...
14. Private instances cannot reach the public internet even though a NAT gateway exists. Most likely?
    ...
    Answer: B — Either the private route table lacks the NAT route, or a device blocks it; verify both.
...
15. An ALB returns 503 Service Unavailable for a backend. This is a signal of which sub-objective?
    ...
    C. Latency (6.2.2)
    D. Routing (6.2.8)
    Answer: B — A 5xx is an HTTP status-code symptom under network service unavailability.
...
16. A VPN tunnel shows healthy but no traffic flows between overlapping address spaces. Cause?
    ...
    D. DHCP
    Answer: B — Overlapping CIDRs make the path ambiguous even when the tunnel is up; re-IP one side.
...
17. Two systems open a TCP connection but the encrypted handshake resets immediately. Cause?
    ...
    C. Scope exhaustion
    D. VLAN misconfiguration
    Answer: B — The path works (TCP opens); the protocol/cipher negotiation fails — incompatibility.
...
18. A connection to a database times out and traceroute ends at the subnet gateway with nothing beyond. Cause?
    ...
    C. DHCP failure
    D. Bandwidth
    Answer: A — Traffic dies at the gateway with no next hop = missing or misconfigured route.
...
19. A newly attached ENI can't reach peers in its intended subnet though its IP/subnet is correct. Cause?
    ...
    C. DNS
    D. Deprecation
    Answer: B — Correct L3 but isolated segment = VLAN tag/access-trunk misconfiguration at Layer 2.
...
20. An application returns 404 Not Found for an API path that exists on another environment. Under 6.2 this is classified as:
    ...
    C. Latency
    D. Bandwidth
    Answer: B — A 4xx is an HTTP status-code symptom of network service unavailability (client/request side).
