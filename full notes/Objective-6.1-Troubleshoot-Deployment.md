...
1. A deployment fails with "AccessDenied" although the resource exists. What is the most likely cause?
   ...
   C. API throttling
   D. Deprecation
   Answer: B — The resource exists but the identity lacks the required IAM/role permission.
...
2. You receive "InsufficientInstanceCapacity" for a specific VM family in one AZ, but your quota is unused. Cause?
   ...
   Answer: C — No free physical capacity for that family in that AZ; move AZ/region or reserve capacity.
...
3. A container built on a dev laptop crashes on the cluster with a GLIBC mismatch. Cause?
   ...
   C. Quota
   D. Partial outage
   Answer: B — Base image/libs or architecture differ from the host runtime; rebuild to match.
...
4. CI pipelines return HTTP 429 "rate limit exceeded" after adding parallel jobs. Cause?
   ...
   Answer: B — Request rate exceeded the API rate limit; add backoff and stagger jobs.
...
5. A Terraform apply fails with "QuotaExceeded" for vCPUs. Cause?
   ...
   D. Oversubscription
   Answer: C — Account/service quota ceiling reached; request a limit increase or free resources.
...
6. A template works in us-east-1 but fails "OperationNotSupportedInRegion" in ap-southeast-1. Cause?
   ...
   D. Throttling
   Answer: B — The feature/instance family is not offered in the target region.
...
7. An app launches but cannot reach its database across subnets. Cause?
   ...
   C. Deprecation
   D. Throttling
   Answer: B — Network/security misconfiguration isolates the resources, not a capacity issue.
...
8. Deployments suddenly fail with "unknown parameter" on an API call that worked last month. Cause?
   ...
   C. Quota
   D. Oversubscription
   Answer: B — The API version/parameter was deprecated or removed; update the call/definition.
...
9. Autoscaler cannot add nodes: "no capacity" in the chosen AZ. Cause?
   ...
   C. Regional/capacity availability
   D. Sizing
   Answer: C — The node instance family has no capacity there; try another AZ/type or reserve.
...
10. A VM experiences high CPU steal and slow disks despite correct sizing. Cause?
   ...
   Answer: B — Host over-allocated (noisy neighbor) starves your VM of physical resources.
...
11. A Kubernetes Helm chart fails on a newly upgraded cluster with CRD errors. Cause?
   ...
   C. Partial outage
   D. Throttling
   Answer: A — Chart/CRD API versions don't match the upgraded cluster version; update the chart.
...
12. You cannot launch a GPU instance in your selected region. Cause?
   ...
   C. Oversubscription
   D. Outdated definition
   Answer: B — GPU families are limited to specific regions/AZs; choose a supported region.
...
13. Intermittent 5xx errors hit only one microservice while others are fine. Cause?
   ...
   C. Quota
   D. Sizing
   Answer: B — A specific dependency or AZ is degraded; check health and fail over.
...
14. A re-deploy fails with "EntityAlreadyExists." Cause?
   ...
   C. Deprecation
   D. Regional availability
   Answer: B — The resource already exists; fix naming/idempotency or import the existing resource.
...
15. Deployments are slow only at month-end peak across teams. Most likely cause?
   ...
   C. Full outage
   D. Incompatibility
   Answer: B — Shared automation account hits rate limits under peak parallel load; stagger/raise limits.
...
16. A template references a subnet that was deleted last quarter and apply fails. Cause?
   ...
   C. Oversubscription
   D. Regional availability
   Answer: A — Stale definition points to removed network resources; refresh IDs and re-validate.
...
17. An instance is under-provisioned and the app crashes under load. Cause?
   ...
   C. Throttling
   D. Deprecation
   Answer: A — Wrong vCPU/RAM sizing; right-size to workload requirements.
...
18. A managed service returns 503 only during a provider-incident window for one region. Cause?
   ...
   C. Incompatibility
   D. Oversubscription
   Answer: B — Provider-side outage (check status page); not a local config problem.
...
19. A build succeeds on x86 CI but pods crash on ARM nodes. Cause?
   ...
   C. Throttling
   D. Quota
   Answer: B — Architecture mismatch; build multi-arch images and set correct node selectors.
...
20. A deployment uses a long-lived access key that now returns "SignatureDoesNotMatch." Recent policy forced IMDSv2/role use. Cause?
   ...
   C. Oversubscription
   D. Sizing
   Answer: B — Auth method deprecated/blocked by new policy; switch to IAM roles/instance profile.
